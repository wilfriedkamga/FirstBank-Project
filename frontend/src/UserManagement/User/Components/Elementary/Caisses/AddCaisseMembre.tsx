...
const AddCaisseMembre = ({setVisibility, visibility}:ChildComponentProps) => {
  ...
  const [membreCaisseModel, setMembreCaisseModel]=useState<TMembreCaisseModel>()
  const [phone, setPhone]=useState("")
  const location=useLocation()
  const [tontineMembreList, setTontineMembreList] = useState<TMembreCaisseModel[]>([]);
    ...
    <div className="">
      <form
        onSubmit={(e: React.FormEvent<HTMLFormElement>) => {
          HandleAdd(e); setVisibility(!visibility)
        }}
      >
        ...
        <div className=" flex mx-4 flex-col gap-1">
          ...
          <div className="flex gap-4 justify-between">
            <div className=" w-full">
              ...
              >
                <option value="value 1">chosir le role</option>
                <option value="validateur">Validateur</option>
                <option value="participant">Participant</option>
              </select>
            </div>
            <div className="w-3/5 ">
              <label className="block mb-2 text-sm mt-3 ">Nombre de noms</label>
              <input
                value={nbNom}
                onChange={(e)=>{setNbNom(e.target.value)}}
                type="number"
                placeholder="Nom par defaut"
                className="bg-transparent w-full rounded-lg px-3 py-2.5 2xl:py-3 border border-gray-300 placeholder-gray-400 text-gray-900 outline-none text-base focus:ring-2 ring-blue-300"
              />
          ...
          <div className="flex justify-end mt-4  text-white ">
            <button
              onClick={() => {setEnd(false); setVisibility(!visibility)}}
...
export default AddCaisseMembre
