...
interface Props {
  data: RemboursementModel[];
}
const ListeDettes = ({ data }: Props) => {
  const columns: GridColDef<RemboursementModel>[] = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box sx={{display:"flex", gap:2}}>
         ...
         <DeleteButton remb={params.row} handleDelete={()=>null}/>
          <DownloadButton remb={params.row} handleDownload={()=>null}/>
          <EditButton remb={params.row} handleSave={()=>null}/>
        </Box>
      ),
    },
  ];
  ...
  return (
    <div>
      ...
      <MUIDatatable<RemboursementModel> columns={columns} data={data} />
    </div>
  );
};
