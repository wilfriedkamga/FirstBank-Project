...
export default function DeleteButton({handleDelete, doc}:props) {
  ...
  const localHandleDelete=(doc:DocumentModel)=>{
    setIsLoading(true)
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <Divider/>
        <DialogActions>
          <Button variant="contained" sx={{backgroundColor:"#c00", "&:hover":{backgroundColor:"#a00"}}} onClick={()=>localHandleDelete(doc)}> {!isLoading?"Oui":<CircularProgress size={20} sx={{color:"white", fontWeight:"bold"}}/>} <Delete/></Button>
