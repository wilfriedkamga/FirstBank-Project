...
const ModifyPassword = () => {
  ...
  const handleClick = (e:React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if(newPassword!=confirmPassword){ alert("les mots de passe ne sont pas identiques")}
    else{
      Authentications.verifyPasswordService(phone, nowPassword)
      .then((response) => {
        changePassword()
      ...
      .catch((error) => {
        alert("La modification n'est pas allée jusqu'au bout")
      });
          ...
          <div className="flex flex-col font-semibold  bg-white mt-2 rounded-lg mb-2 mx-auto w-full md:w-2/5 p-2 md:p-5 shadow-lg border h-full">
            ...
            <div className="flex w-full flex-col">
              ...
              <div className="flex flex-row  h-[20px] mt-2 relative ml-2">
                  <input type="checkbox" onChange={()=>setPasswordVisibility(!passwordVisibility)} className="border  h-[20px] w-[20px] " />
                  <label htmlFor="" className="ml-2 relative bottom-[2px]  ">Voir</label>
              </div>
