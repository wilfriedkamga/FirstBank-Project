...
const Details = () => {
  ...
  const deletePlan = () => {
    if (plan) {
      ...
      axios
        .delete(`http://localhost:8085/savingsplanManagement/delete-saving`, {
          data: saving,
        })
        .then((response) => {
          console.log(response.data);
  ...
  return (
    <div className="w-full bg-white h-full flex flex-col">
      ...
      <div className="flex mt-[10vh] mb-[20vh] p-2 flex-col pt-5 h-full w-full space-y-8 overflow-auto">
        {plan && (
          <div className="h-3/4">
            ...
            <div className="w-full relative flex flex-col bg-white rounded-lg mt-5 h-full p-4 font-title">
              ...
              <div className="flex justify-between h-20 items-center mb-5">
                ...
                <div className="text-2xl">{plan.amountTarget}</div>
              </div>
              <div className="flex justify-between h-20 items-center mb-5">
              ...
              <div className="flex absolute md:w-1/2 w-full justify-between bottom-0 right-0 p-4 space-x-5">
                ...
                <div className="bg-[#BB0A01] rounded w-1/3 h-12 font-title text-white py-3 text-center">
                  {" "}
                  Top-up / Withdraw{" "}
                </div>
                <div
      ...
      {plan && showEditPopover && (
        <div
          className={`absolute z-20 top-0 left-0 w-full h-full flex justify-center items-center bg-gray-500 bg-opacity-50 ${
            showEditPopover ? "opacity-100" : "opacity-0"
          } transition duration-300 ease-in-out`}
        >
          <div
            className={`bg-white h-3/4 font-title mx-auto relative max-w-sm overflow-hidden md:max-w-7xl rounded-md p-4 w-full ${
              showEditPopover ? "cale-100" : "cale-50"
      ...
      {plan && showDeletePopover && (
        <div
          className={`absolute z-20 top-0 left-0 w-full h-full flex justify-center items-center bg-gray-500 bg-opacity-50 ${
            showDeletePopover ? "opacity-100" : "opacity-0"
          } transition duration-300 ease-in-out`}
        >
          <div
            className={`bg-white h-1/4 font-title mx-auto max-w-sm md:max-w-6xl rounded-md p-4 w-full ${
              showDeletePopover ? "cale-100" : "cale-50"
            } transition flex flex-col duration-300 ease-in-out`}
          >
