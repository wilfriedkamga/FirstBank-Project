...
const CreatePlan = () => {
  ...
  const [finishType, setFinishType] = useState('early')
  const [isFinishTypeChanged, setIsFinishTypeChanged] = useState(false);
  ...
  const handleFinishTypeChange = (finishType : string) => {
    setFinishType(finishType);
    setIsDecimal(false);
  }

  const handleFrequentAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const submitForm = () => {
    if (reason && target && dueDate && selectedDates && startDate){
      ...
      axios.post(`${Variable.saving_base_url}/savingsplanManagement/create-savings`, savings)
        .then(response => {
  ...
  return (
    <div className='w-full h-full flex flex-col'>
      ...
      <div className="flex h-full mt-[10vh] p-2 overflow-auto overscroll-contain">
        <div className="flex flex-col md:border-r pt-5 h-full w-full space-y-8">
          <div className="pt-5 ml-5 border-b flex justify-between border-gray-200">
            ...
            <div className="mr-5 mb-2 py-2.5 px-5 bg-[#BB0A01] rounded-full text-white font-title cursor-pointer" onClick={validateForm}>
              Next
            </div>
          </div>
            ...
            <div className="flex flex-col mb-8">
              <div className="md:flex-1 mt-2 md:mt-0 md:px-3">
                <div className="p-2.5">
                  ...
                  <input className="w-full shadow-inner font-title p-4 rounded-lg border-0 focus:outline-red-500" type="text" name="reason" value={reason} placeholder="Buy A Computer" onChange={handleReasonChange}/>
                  {errors.reason && <div className="text-red-500 font-title text-sm">{errors.reason}</div>}
                </div>
                <div className="md:flex w-full p-2.5">
                  <div className="mb-2 w-full pr-2">
                    ...
                    <input className="w-full shadow-inner font-title p-4 rounded-lg border-0 focus:outline-red-500" type="number" name="Target amount" placeholder="237650" value={target} onChange={handleTargetAmountChange}/>
                    {errors.target && <div className="text-red-500 font-title text-sm">{errors.target}</div>}
                  </div>
                  <div className="mb-2 w-full px-2">
                    <label className="text-md font-title font-medium ml-5">Starting Date</label>
                    <input className="w-full shadow-inner p-4 rounded-lg border-0 bg-white font-title focus:outline-red-500" type="date" name="startDate" value={startDate? startDate.toISOString().substring(0,10) : ''} onChange={handleStartDateChange}/>
                    {errors.startDate && <div className="text-red-500 font-title text-sm">{errors.startDate}</div>}
                  </div>
                  <div className="mb-2 w-full pl-2">
                    <label className="text-md font-title font-medium ml-5">Due date</label>
                    <input className="w-full shadow-inner p-4 rounded-lg border-0 bg-white font-title focus:outline-red-500" type="date" name="dueDate" value={dueDate? dueDate.toISOString().substring(0,10) : ''} onChange={handleDueDateChange}/>
                    {errors.dueDate && <div className="text-red-500 font-title text-sm">{errors.dueDate}</div>}
                  </div>
                </div>
                <div className="md:flex w-full p-2.5">
                  <div className="w-full pr-2">
                    <label className="text-md font-title font-medium ml-5">Frequent amount</label>
                    <input className="w-full shadow-inner font-title p-4 rounded-lg border-0 focus:outline-red-500" type="number" name="Frequent amount" placeholder="237650" value={frequentAmount} onChange={handleFrequentAmountChange}/>
                    {errors.frequentAmount && <div className="text-red-500 font-title text-sm">{errors.frequentAmount}</div>}
                  </div>
                  <div className="w-full pl-2">
                    ...
                    {errors.selectedDates && <div className="text-red-500 font-title text-sm">{errors.selectedDates}</div>}
                  </div>
                </div>
                ...
                <div className="w-full p-2.5">
                    ...
                    <div className="flex flex-wrap">
                      {selectedDates.map((date) => (
                        <div key={date.toISOString()} className="bg-blue-500 text-white p-2 font-title rounded-lg m-1">
            ...
            <div className={`bg-white h-fit font-title mx-auto relative max-w-sm overflow-hidden md:max-w-5xl rounded-md p-4 w-full ${isDecimal
              ...
              : "cale-50"} transition flex flex-col duration-300 ease-in-out`}>
                ...
                <div className="w-full h-full flex p-2.5 space-x-3">
                  <div className="w-full text-white text-center py-20 bg-[#BB0A01] rounded-md cursor-pointer   " onClick={() => {handleFinishTypeChange('early')}}>Early Finish</div>
                  <div className="w-full text-white text-center py-20 bg-[#BB0A01] rounded-md cursor-pointer" onClick={() => {handleFinishTypeChange('late')}}>Late Finish</div>
                </div>
              </div>
          </div>
      )}
      {preview && (
        <div className={`absolute z-20 top-0 left-0 w-full h-full flex justify-center items-center bg-gray-500 bg-opacity-50 ${preview
          ? "opacity-100"
          : "opacity-0"} transition duration-300 ease-in-out`}>
          <div className={`bg-white h-fit font-title mx-auto relative max-w-sm overflow-hidden md:max-w-5xl rounded-md p-4 w-full ${isDecimal
              ? "cale-100"
              : "cale-50"} transition flex flex-col duration-300 ease-in-out`}>
                <div className="flex justify-between mb-8">
                  <h1 className="text-2xl font-title font-semibold">Preview & Save</h1>
                  <div className="cursor-pointer" onClick={() => setPreview(false)}>
                    <XMarkIcon className="h-8 w-8" />
                  </div>
                </div>
                <p className="text-sm mb-5">You are about to create a new plan with the following details:</p>
                <div className="flex flex-wrap mb-6 overflow-y-auto">
                  <div className="w-full p-2.5">
                    <label className="text-md font-title font-medium ml-5">Plan Reason</label>
                    <input className="w-full shadow-inner font-title p-4 rounded-lg border-0 focus:outline-red-500" type="text" name="reason" value={reason}/>
                  </div>
                  <div className="md:flex w-full p-2.5">
                    <div className="w-full pr-2">
                      <label className="text-md font-title font-medium ml-5">Target</label>
                      <input className="w-full shadow-inner font-title p-4 rounded-lg border-0 focus:outline-red-500" type="text" name="target" value={target}/>
                    </div>
                    <div className="w-full pl-2">
                      <label className="text-md font-title font-medium ml-5">Frequent Amount</label>
                      <input className="w-full shadow-inner font-title p-4 rounded-lg border-0 focus:outline-red-500" type="text" name="frequent" value={frequentAmount}/>
                    </div>
                  </div> 
                  <div className="md:flex w-full p-2.5">
                    <div className="mb-2 w-full pr-2">
                      <label className="text-md font-title font-medium ml-5">Starting Date</label>
                      <input className="w-full shadow-inner p-4 rounded-lg border-0 bg-white font-title focus:outline-red-500" type="date" name="startDate" value={startDate? startDate.toISOString().substring(0,10) : ''}/>
                    </div>
                    <div className="mb-2 w-full pl-2">
                      <label className="text-md font-title font-medium ml-5">Due date</label>
                      <input className="w-full shadow-inner p-4 rounded-lg border-0 bg-white font-title focus:outline-red-500" type="date" name="dueDate" value={dueDate? dueDate.toISOString().substring(0,10) : ''}/>
                    </div>
                  </div>
                  <div className="w-full p-2.5">
                    <label className="text-md font-title font-medium ml-5">Selected Dates</label>
                ...
                <div className="w-full bg-white flex justify-between absolute bottom-0 pr-5">
                  <div className="py-2.5 px-10">
                    <input type="checkbox" name="" id="usagecondition" onChange={(e)=> setIsChecked(e.target.checked)}/>
                    <label htmlFor="usagecondition" className='text-sm'> I accept and agree to the Terms of Use</label>
                  </div>
                  <div
                    className={`mr-5 mb-2 md:py-3 py-6 px-10 bg-[#BB0A01] rounded-full w-fit h-5 md:h-12 text-nowrap text-white font-title ${isChecked ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'} cursor-pointer`}
