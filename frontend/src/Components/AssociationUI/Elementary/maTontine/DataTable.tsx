    ...
    <div>
      ...
      >
        ...
        <Button
          ...
          color="primary"
          disabled={data.length >= nombre_max || buttonLoading}
        >
          {buttonLoading ? (
            <CircularProgress size={24} />
          ) : (
          ...
          <TableBody>
            {sortedData.map((member, index) => (
              <TableRow key={member.id}>
                ...
                <TableCell>
                {!member.stateConfirmation&&<ButtonComponent type="error" text="Annuler"/>}
                {member.stateConfirmation && !member.statusConfirmation?<ButtonComponent type="error" text="Renvoyer"/>:<ButtonComponent disabled={true} type="warning" text="Terminé"/>}
