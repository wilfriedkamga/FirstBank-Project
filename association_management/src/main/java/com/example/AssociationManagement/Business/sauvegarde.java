 ...
 *         createRole(association2.getAssociationId(), "createur", false,1);
 *
 *         for (String role : defaultRoles) {
 *             createRole(association2.getAssociationId(), role, true,1000);
 *         }
 *
 *         for (String role : uniqueRoles) {
 *             createRole(association2.getAssociationId(), role, false,1);
 *         }
 ...
 *                 .map(role -> new RoleAssoDto(role.getAssociationId(), role.getLabel(), role.getNbMaxOcc()))
 ...
 *         CreateAssoDto createAssoDto = new CreateAssoDto(association.getAssociationId(), association.getName(), association.getFrequenceReunion(), association.getJourReunion(), association.getCreationDate(), roles, membres,3,1,0);
 *
 *         CreateTontineModele createTontineModele=new CreateTontineModele();
 *         createTontineModele.setAssociationId(association2.getAssociationId());
 ...
 *                 .collect(Collectors.toList());
 *
 *         AssociationDto associationDto=new AssociationDto(association);//association.getAssociationId(),association.getName(),association.getFrequenceReunion(),association.getJourReunion(),association.getCreationDate(),association.getNbMembers(),association.getNbTontines(),association.getEvenements().size(),association.getReunions().size(),association.getDocument().size());
 *
 *
 *         return associationDto;
 *     }
 ...
 *             Map<String ,  O bject> userDetails = (Map<String, Object>) response.getData();
 ...
 *             roleIdList.add(association.getRoles().get(i).getAssociationId());
 *             membreAssoList.add(association.getMembres().get(i).getAssociationId());
 *         }
 ...
 *             if(role.getAssociationId().equals(roleId)){
 *                 roleIndex.set(roleIndex2.get());
 *             }
 *         });
 *
 *         if(roleExist.get()){
 *             roleAssoRepository.deleteById(roles.get(roleIndex.get()).getAssociationId());
 ...
 *                 roleAssoRepository.deleteById(roles.get(i).getAssociationId());
 *             }
 *         }
 *     }
 *
 ...
 *
 *         CreateAssoDto createAssoDto = new CreateAssoDto(association.getAssociationId(), updateAssoModel.getName(), updateAssoModel.getFrequenceReunion(), updateAssoModel.getJourReunion(), association.getCreationDate(), roles, membres,membres.size(),association.getNbTontines(),association.getNbTontines());
 *         associationRepository.save(association);
 *
 ...
 *             tontineDto.setAssociationId(tontine.getAssociationId());
 ...
 *                 .map(reunion -> new ReunionDto(reunion.getAssociationId(), reunion.getDateSeance()))
 *                 .collect(Collectors.toList());
 *     }
 *
 ...
 *                 .map(event -> new EventDto(event.getAssociationId(), event.getDescription(), event.getDateEcheance()))
 *                 .collect(Collectors.toList());
 *     }
 *
 *
 ...
 *
        ...
        *if(listMembres.get(i).getAssociations().get(j).getAssociationId().equals(createTontineModel.getAssociationId())){
        ...
        * //
        * //        addMembreTontine(newTontine.getAssociationId(), validateurRole.getAssociationId(), createTontineModel.getPhoneValidateur1());
        * //        addMembreTontine(newTontine.getAssociationId(), validateurRole.getAssociationId(), createTontineModel.getPhoneValidateur2());
        *
        ...
        *addMembreTontine(newTontine.getAssociationId(),validateurRole.getAssociationId(),createTontineModel.getPhoneValidateur1());
        *addMembreTontine(newTontine.getAssociationId(),validateurRole.getAssociationId(),createTontineModel.getPhoneValidateur2());
        *addMembreTontine(newTontine.getAssociationId(),validateurRole.getAssociationId(),createTontineModel.getPhoneCreator());
        *
        *return null;// newTontine;
        *}
        *
        ...
        *associationRepository.save(association);
        *documentDto.setAssociationId(document.getAssociationId());
        ...
        *docDto.setAssociationId(doc.getAssociationId());
