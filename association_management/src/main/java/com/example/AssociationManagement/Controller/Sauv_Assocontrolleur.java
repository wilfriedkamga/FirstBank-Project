...
public class Sauv_Assocontrolleur {
    ...
    @PostMapping("/repondre_invitation")
    public ResponseEntity<?> answerInvitation(@RequestBody InvitationResponseModel model) {
        // Créer et retourner le DTO
        return ResponseEntity.ok(associationService.answerNotification(model.getMembre_id(),model.isAnswer()));
    }
    ...
    public ResponseEntity<?> createRoleInAssociation(@RequestBody RoleCreationModel roleCreationModel) {
        ...
        RoleAssoDto roleAssoDto=new RoleAssoDto(role.getAssociationId(),role.getLabel(),role.getNbMaxOcc());
    ...
    public ResponseEntity<?> getDetails(@RequestParam String associationId) {
        ...
        List<RoleAssoDto> rolesDto = roles.stream()
                .map(role -> new RoleAssoDto(role.getAssociationId(), role.getLabel().toUpperCase(), role.getNbMaxOcc()))
                .collect(Collectors.toList());
        return ResponseEntity.ok().body(rolesDto);
    }
